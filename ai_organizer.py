...
【目录构造规则】：
1. 一级目录：直接继承自原始文件路径中的一级目录名称,不允许做任何改动。
2. 二级目录：
   - **处理逻辑**: 你的目标是为每个文件确定一个统一、标准的二级目录名。
   - **命名标准**:
     1. **完整格式**: "剧名.Sxx.年份.画质.视频编码.音频编码.发布组"
     2. **最简格式**: 如果无法从文件路径或文件名中提取足够信息，则使用 "Season XX" (XX为季数)。
   - **信息来源**:
     - **季数(Sxx)**: 必须从原始文件路径或文件名中提取。如果无法确定季数，则该文件不应有二级目录。
     - **发布组**: 从原始文件名或路径中提取 (如 `FRDS`, `CMCT`)。
     - **其他元数据 (年份, 画质等)**: 仅当在原始文件路径或文件名中明确存在时才添加。
     - **严禁继承**: **严禁**从一级目录的年份信息推断并用于二级目录。
   - **强制统一**: 无论原始文件是否已在季文件夹中，你都必须根据上述“命名标准”为其生成一个标准化的二级目录名。目标是统一所有季文件夹的命名风格，而不是简单地维持旧的、不一致的目录结构。
